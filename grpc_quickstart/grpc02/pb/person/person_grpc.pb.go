...
type SearchServiceClient interface {
	...
	SearchInt(ctx context.Context, opts ...grpc.CallOption) (SearchService_SearchIntClient, error)
...
func (c *searchServiceClient) SearchInt(ctx context.Context, opts ...grpc.CallOption) (SearchService_SearchIntClient, error) {
	stream, err := c.cc.NewStream(ctx, &SearchService_ServiceDesc.Streams[0], "/person.SearchService/SearchInt", opts...)
	if err != nil {
		return nil, err
	}
	x := &searchServiceSearchIntClient{stream}
	return x, nil
}

type SearchService_SearchIntClient interface {
	Send(*Person) error
	CloseAndRecv() (*Person, error)
	grpc.ClientStream
}

type searchServiceSearchIntClient struct {
	grpc.ClientStream
}

func (x *searchServiceSearchIntClient) Send(m *Person) error {
	return x.ClientStream.SendMsg(m)
}

func (x *searchServiceSearchIntClient) CloseAndRecv() (*Person, error) {
...
type SearchServiceServer interface {
	...
	SearchInt(SearchService_SearchIntServer) error
...
func (UnimplementedSearchServiceServer) SearchInt(SearchService_SearchIntServer) error {
	return status.Errorf(codes.Unimplemented, "method SearchInt not implemented")
}
...
func _SearchService_SearchInt_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(SearchServiceServer).SearchInt(&searchServiceSearchIntServer{stream})
}

type SearchService_SearchIntServer interface {
	SendAndClose(*Person) error
	Recv() (*Person, error)
	grpc.ServerStream
}

type searchServiceSearchIntServer struct {
	grpc.ServerStream
}

func (x *searchServiceSearchIntServer) SendAndClose(m *Person) error {
	return x.ServerStream.SendMsg(m)
}

func (x *searchServiceSearchIntServer) Recv() (*Person, error) {
	m := new(Person)
...
var SearchService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "SearchInt",
			Handler:       _SearchService_SearchInt_Handler,
			ClientStreams: true,
		},
		{
